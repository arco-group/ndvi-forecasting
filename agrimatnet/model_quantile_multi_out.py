...
class AgriMatNetQuantile(nn.Module):
    ...
    def __init__(
        ...
        if num_targets < 1:
            raise ValueError("num_targets deve essere >= 1.")
...
def quantile_loss(preds, targets, mask, quantiles, weights=None):
    ...
    con k numero di indici da predirre
    preds: (B, T, K, Q)
    targets: (B, T, K)
    mask: (B, T, K) True sui valori da ignorare
    ...
    if weights is not None:
        if weights.shape != keep.shape[:2]:
            raise ValueError("weights deve avere shape (B, T).")
        weight_tensor = weight_tensor * weights.unsqueeze(-1)
    targets = targets.unsqueeze(-1)  #  (B, T, K, 1)
    diff = targets - preds  # (B, T, K, Q)
    ...
    loss_stack = torch.stack(losses, dim=-1)  # (B, T, K, Q)
